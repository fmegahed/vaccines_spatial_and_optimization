...
- Fit a spatial model to explain the differences in the $\phi_1, \phi_2$, and $\phi_3$ parameters obtained from the `r scales::comma(nrow(readRDS("data/p_matrix_final.rds")))` logistic growth curve models for the counties with vaccination data starting from March 01, 2022. **Note that INLA also allows us to impute missing data in both the responses and the covariates** since: (a) ["Bayesian methods may handle complex models that are difficult to fit using classical methods such as repeated measures, missing data, and multivariate data''](https://www.paulamoraga.com/book-geospatial/sec-bayesianinference.html?q=missing#bayesian-inference); and (b) ["INLA will automatically compute the predictive distributions for all missing values in the response, which should be assigned a NA value when defining the data"](https://becarioprecario.bitbucket.io/inla-gitbook/ch-missing.html). 
