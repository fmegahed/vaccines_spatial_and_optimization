...
In the code chunk below, we read the 
We can impute the unknown data by utilizing the [CDC's COVID-19 Vaccinations in the United States,Jurisdiction dataset](https://data.cdc.gov/Vaccinations/COVID-19-Vaccinations-in-the-United-States-Jurisdi/unsk-b7fc) and identify the dates for which the series is non-zero. 
...
cdc_state_vacc %>% 
  ...
  DT::datatable(options = list(scrollX = '300px'))
...
_ aggregated the [COVID-19 Vaccinations in the United States by County](https://data.cdc.gov/Vaccinations/COVID-19-Vaccinations-in-the-United-States-County/8xkx-amqh) dataset, by summing the `series_complete_yes` for all counties within a given state; and  
...
In the code chunk below, we compute the number of observations (`n`) where the `series_complete_pop_pct` was non-zero. It would not be unusual to see a zero on `2021-01-01` since the [first COVID-19 administered in the U.S.](https://www.wsj.com/articles/covid-19-vaccinations-in-the-u-s-slated-to-begin-monday-11607941806) was on `2020-12-14`. However, we hypothesize that most counties should have vaccinated at least one individual by `2021-02-01`. 
...
From the computations above, the distribution of missing observations per county (i.e., ones where there are zero observations per county) is shown below. Note that the top row corresponds to the observed number of missing observations per county, and the bottom row corresponds to its frequency. 
...
leaflet_pal =  colorNumeric('Oranges', 
                            domain = leaflet_sf$num_missing, 
...
leaflet() %>% # initializing the leaflet map
  ...
  addPolygons(
    data = leaflet_sf, stroke = FALSE, fillColor = ~leaflet_pal(leaflet_sf$num_missing), # adding the data
  ...
  addLegend(position = "bottomright", pal = leaflet_pal, values =  leaflet_sf$num_missing, 
            title = "#Obs Missing", opacity = 1) # legend formatting
```
...
tm_shape(cty_sf_missing) + 
  tm_polygons('num_missing', title = '#Obs Missing', palette = "seq") +
  tm_shape(states_sf) + 
  tm_borders(col = "black", lwd = 0.5) +
  tm_layout(aes.palette = list(seq = "-Oranges"))
